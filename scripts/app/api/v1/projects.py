...
from app.schemas.project import ProjectMemberBrief
...
async def build_project_response(db: AsyncSession, project: Project) -> ProjectResponse:
    """
    Helper function to build ProjectResponse with member info.
    ...
    # Get members for this project (limited to first 5 for display)
    members_query = (
        select(ProjectMember)
        .options(selectinload(ProjectMember.user))
        .where(ProjectMember.project_id == str(project.id))
        .limit(5)
    )
    ...
    member_count_query = select(func.count()).select_from(ProjectMember).where(
    ...
    for member in members:
        members_brief.append(ProjectMemberBrief(
            id=str(member.id),
...
async def list_projects(
    ...
    # Add document counts and member info to each project
    from sqlalchemy import select, func
    from sqlalchemy.orm import selectinload
    from app.models.document import Document
    from app.models.project_member import ProjectMember
    ...
    for project in projects:
        ...
        # Get members for this project (limited to first 5 for card display)
        members_query = (
            select(ProjectMember)
            .options(selectinload(ProjectMember.user))
            .where(ProjectMember.project_id == str(project.id))
        ...
        # Create response with document count and members
        project_dict = {
            "id": str(project.id),
            "name": project.name,
            "code": project.code,
...
async def complete_project(
    ...
    project.is_active = True  # Still active, just completed
    db.add(project)
    await db.commit()
    await db.refresh(project)
